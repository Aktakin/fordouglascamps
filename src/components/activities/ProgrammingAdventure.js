...
function TopicLearning({ language, topic, currentLevel, setCurrentLevel, setCurrentTopic }) {
  ...
  const nextLevel = (isOverride = false) => {
    if (currentLevel < 10) {
      // If using admin override, mark current level as completed
      if (isOverride) {
        const activityId = `programming-${language}-${topic}`;
        completeLevel(activityId, currentLevel);
      }
  ...
  // Handle triple-tap for admin override
    ...
    <div className="programming-adventure">
      ...
      <div className="programming-container">
        ...
        <div className="code-editor-area">
          ...
          {currentLevel < 10 ? (
            <div style={{ marginTop: levelComplete ? '0' : '1.5rem', textAlign: 'center' }}>
              <button 
                onClick={handleNextLevelClick} 
                className={`next-level-programming-btn ${!levelComplete ? 'disabled-override' : ''}`}
                style={!levelComplete ? { 
                  opacity: 0.5, 
                  cursor: 'pointer',
              ...
              >
                {levelComplete ? `Continue to Level ${currentLevel + 1} →` : `Next Level ${currentLevel + 1} →`}
              </button>
            </div>
          ) : levelComplete && (
